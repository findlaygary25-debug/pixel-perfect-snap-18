...
export function PromotionalBannerCard({ banner, getItemIcon }: PromotionalBannerCardProps) {
  ...
  return (
    <Card className={`relative overflow-hidden border-2 hover:shadow-lg transition-all`}>
      <div className={`absolute inset-0 bg-gradient-to-r ${banner.background_gradient} opacity-10`} />
      <CardContent className="relative pt-6 pb-6">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
          <div className={`p-4 rounded-xl bg-gradient-to-br ${banner.background_gradient} text-white shadow-lg`}>
          ...
          {banner.cta_text && (
            <Button size="lg" className={`bg-gradient-to-r ${banner.background_gradient} hover:opacity-90`}>
              {banner.cta_text}
            </Button>
          )}
        </div>
